...
export function addFrame(rms: number[]): void {
  if (_phase === 'baseline') _baselineBuffer.push(rms.slice(0, CHANNELS));
  else if (_phase === 'mvc') _mvcBuffer.push(rms.slice(0, CHANNELS));
}
...
export function finalizeBaseline(): number[] {
  ...
  const result = computeChannelMean(_baselineBuffer);
  _phase = 'idle';
  return result;
}
...
export function finalizeMVC(): number[] {
  if (_mvcBuffer.length < MVC_PEAK_WINDOW)
    throw new Error(`[NormalizationService] Not enough MVC frames. Need ${MVC_PEAK_WINDOW}, got ${_mvcBuffer.length}.`);
  ...
  for (let i = 0; i <= _mvcBuffer.length - MVC_PEAK_WINDOW; i++) {
    ...
    const mean = computeChannelMean(window);
    const sum = mean.reduce((a, b) => a + b, 0);
    if (sum > bestSum) { bestSum = sum; bestWindow = window; }
  }
  const mvc = computeChannelMean(bestWindow);
  _phase = 'idle';
  return mvc;
}

export function buildCoefficients(baseline: number[], mvc: number[]): CalibrationCoefficients {
...
export function normalize(liveRMS: number[], coeffs: CalibrationCoefficients): number[] {
...
function computeChannelMean(frames: number[][]): number[] {
  const numFrames = frames.length;
  const result = new Array(CHANNELS).fill(0);
  for (const frame of frames) {
    for (let ch = 0; ch < CHANNELS; ch++) {
      result[ch] += (frame[ch] ?? 0) / numFrames;
    }
  }
  return result;
}