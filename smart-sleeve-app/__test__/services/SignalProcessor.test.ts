...
describe("SignalProcessor Service", () => {
  ...
  it("should report warmup only after enough frames have been processed", () => {
    expect(processor.isWarmedUp()).toBe(false);

    for (let index = 0; index < 49; index += 1) {
      processor.processEMG(createPacket(0.1, index));
    }

    expect(processor.isWarmedUp()).toBe(false);

    processor.processEMG(createPacket(0.1, 49));
    expect(processor.isWarmedUp()).toBe(true);
  ...
  it("should have low latency (<20ms)", () => {
    ...
    console.log(
      `Average processing time per packet: ${perPacket.toFixed(4)}ms`,
    );
