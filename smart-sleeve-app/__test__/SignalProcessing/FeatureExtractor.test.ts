...
describe("FeatureExtractor", () => {
  ...
  test("RMS of a simple alternating wave (square-like)", () => {
    ...
    const samples = Array(10)
      .fill(0)
      .map((_, i) => Array(numChannels).fill(i % 2 === 0 ? 1 : -1));
    const rms = FeatureExtractor.calculateRMS(samples);
    expect(rms).toEqual(Array(numChannels).fill(1));
  });
  ...
  test("extractFeaturesWithNormalization returns percent MVC when calibration is provided", () => {
    const samples = [
      [0.8, 0.2, 0, 0],
      [0.8, 0.2, 0, 0],
      [0.8, 0.2, 0, 0],
    ...
    expect(features.rmsNormalized).toEqual([100, 100, 0, 0]);
  });
});
