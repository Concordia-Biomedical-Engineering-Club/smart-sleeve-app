...
export default function DebugDB() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Issues #54 & #7 — expo-sqlite verification</Text>
      ...
      <View style={styles.btnRow}>
        ...
        <TouchableOpacity
          style={[styles.btn, { backgroundColor: sessionStatus === 'RECORDING' ? '#0B74E6' : '#999' }]}
          ...
          <ScrollView style={styles.dataBox}>
            {sessions.map(s => (
              <View key={s.id} style={styles.sessionCard}>
                ...
                <Text style={styles.sessionMeta}>{s.exerciseType} | {s.side} | {s.duration}s</Text>
                <Text style={styles.sessionMeta}>
                  Avg Activation: {s.analytics.avgActivation.toFixed(1)}% | EMG Samples: {sampleCounts[s.id] ?? '…'}
                </Text>
...
const styles = StyleSheet.create({
  ...
  dataBox: { maxHeight: 200, backgroundColor: '#fff', borderRadius: 12, padding: 12 },
  sessionCard: { marginBottom: 12, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#E1E3E5' },
  sessionId: { fontSize: 10, fontFamily: 'monospace', color: '#666', marginBottom: 2 },
  sessionMeta: { fontSize: 12, color: '#1A1A1A', marginBottom: 1 },
  sessionSync: { fontSize: 12, fontWeight: '600', marginTop: 2 },
  logBox: { backgroundColor: '#1A1A1A', borderRadius: 12, padding: 12, maxHeight: 180 },
